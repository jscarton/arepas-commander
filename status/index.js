...
console.log(chalk.cyan('INFO:')+ 'querying status of Arepas Developer Environment (ADE) at '+cwd+'/'+project);

//Setup ADE specific platform
try {
  fs.accessSync(project, fs.constants.R_OK);
  shell.cd(project);
  if (shell.exec("vagrant status").code !== 0) {
    console.log(chalk.red('ERROR:')+'sorry, vagrant has returned a non zero exit code, please check output to see what happened');
    shell.exit(1);
  }
} catch (e) {
    console.log(chalk.cyan('ERROR:')+ 'trying to get status of ADE for '+project.toUpperCase()+", please ensure you have initialized this ADE");
}